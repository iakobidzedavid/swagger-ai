...
export function normalizeStoredProduct(row: StoredProductRow, index: number = 0): NormalizedStorefrontProduct {
  ...
  const priceInDollars = typeof storedPrice === 'number' && !Number.isNaN(storedPrice)
    ? storedPrice
    : fallback?.variants[0]?.price ?? 19.99

  // Convert price from dollars to cents for display/cart operations
  ...
  const variants =
    Array.isArray(row.variants) && row.variants.length > 0
      ? row.variants.map(v => ({
          id: v.id,
          title: v.title,
          price: Math.round(typeof v.price === 'number' ? v.price * 100 : 1999),
        }))
      : [{ id: 'v1', title: 'Standard', price: priceInCents }]
